Perfeito — abaixo está **o conteúdo do README.md com a formatação corrigida**, pronto para **copiar, colar e subir no repositório**.
Não alterei o texto, apenas arrumei Markdown, blocos de código e links.
...
- **Tema:** [Spaceship Prompt](https://spaceship-prompt.sh/)  
  Minimalista, mostra versões do Python, Docker, Node e status do Git
...
```bash
git clone https://github.com/SEU_USUARIO/terminal-setup.git
cd terminal-setup
````
...
> **Nota:** O script pedirá sua senha de `sudo` para instalar os pacotes básicos
> (git, curl, zsh, tilix, fontes).
...
Após rodar o script e abrir o **Tilix**, é possível que os ícones do tema apareçam como quadrados brancos (`▯`).
Isso acontece porque o terminal precisa ser configurado manualmente para usar a fonte correta.
...
6. Clique no nome da fonte e selecione **Fira Code Regular**
   *(recomendado tamanho 12)*
...
* `install.sh`
  Script principal (Bash). Instala dependências, baixa o Oh My Zsh e configura os links simbólicos.
  É **idempotente** (pode ser executado várias vezes para corrigir ou atualizar a instalação).

* `zshrc_custom`
  Arquivo de configuração modelo. O script copiará este arquivo para o seu `~/.zshrc`.
...
Se quiser, posso:
- adaptar o README para **outras distros** (Pop!_OS, Fedora, Arch)
- adicionar **badges do GitHub**
- ou revisar o `install.sh` para garantir 100% de idempotência 😉
```
